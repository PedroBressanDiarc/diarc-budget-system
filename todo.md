...
## 📥 IMPORTAÇÃO AUTOMÁTICA DE PERMISSÕES DA PLANILHA

### Problema
- Permissões não estão completas no banco
- Manutenção deveria ver "Manutenções" mas não está aparecendo
- Precisa importar da planilha Excel automaticamente

### Tarefas
- [x] Criar script Node.js para importar permissões da planilha
- [x] Mapear ABA 1 (Menu Sidebar) para permissões no banco
- [x] Limpar permissões antigas e inserir novas
- [x] Executar script (8 permissões para manutenção)
- [x] Corrigir bug: getUserPermissionsByRole retornava [] ao invés de null
- [x] Corrigir useRolePermissions para tratar null como acesso total (Diretor)
- [ ] Testar com usuário Manutenção
- [ ] Criar checkpoint
